...
export function TickerWorkbench({ ticker, onNavigate, guestMode = false }: { ticker: string; onNavigate?: (panel: string, ticker?: string) => void; guestMode?: boolean }) {
  ...
  return <div data-guest-panel={guestMode ? "Ticker Workbench" : undefined} data-guest-source={guestMode ? (quote ? "live" : "loading") : undefined} className="space-y-4" style={{ fontFamily: "var(--font-mono)" }}>
    ...
    <div role="tablist" aria-label="Ticker workbench views" className="flex gap-1 overflow-x-auto rounded-lg border p-1" style={{ borderColor: "var(--line)", background: "var(--panel)" }}>
      {visibleTabs.map((item, index) => <button key={item} ref={(node) => { tabRefs.current[index] = node; }} id={`workbench-tab-${item.toLowerCase()}`} role="tab" aria-controls={`workbench-panel-${item.toLowerCase()}`} aria-selected={tab === item} tabIndex={tab === item ? 0 : -1} onKeyDown={(event) => moveTab(event, index)} onClick={() => setTab(item)} className="min-w-fit rounded-md px-3 py-1.5 text-[10px] font-semibold" style={{ background: tab === item ? "var(--nav-active)" : "transparent", color: tab === item ? "var(--text)" : "var(--text-mute)" }}>{item}</button>)}
    </div>
    <div role="tabpanel" id={`workbench-panel-${tab.toLowerCase()}`} aria-labelledby={`workbench-tab-${tab.toLowerCase()}`}>
    {tab === "Overview" && <div className="space-y-4">
      {guestMode ? <section aria-label="Guest data access" className="rounded-xl border p-4" style={{ borderColor: "var(--gold)", background: "var(--panel)" }}>
        <h2 className="text-sm font-semibold">Guest market preview</h2>
        <p className="mt-1 text-[10px]" style={{ color: "var(--text-mute)" }}>Quote, chart, options-chain, Night Vision, and Strike Matrix views are available for the guest symbol set. Private research, saved state, flow, company context, and agent tools require a signed-in profile.</p>
      </section> : <section aria-label="Ticker data readiness" className="rounded-xl border p-4" style={{ borderColor: "var(--line)", background: "var(--panel)" }}>
