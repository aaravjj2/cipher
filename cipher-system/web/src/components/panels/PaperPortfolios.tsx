...
export function PaperPortfolios() {
  ...
  if (!data) return <div style={{ color: "var(--text-mute)" }}>Loading six shadow portfolios…</div>;
  return (
        ...
        {data.portfolios.map((portfolio) => (
          <details key={portfolio.portfolio_id} className="rounded-xl border p-4" style={{ borderColor: "var(--line)", background: "var(--panel)" }}>
            <summary className="cursor-pointer list-none">
              <div className="flex items-start justify-between gap-4"><div><div className="flex flex-wrap items-center gap-2"><span className="text-sm font-semibold">{portfolio.strategy}</span>{portfolio.risk_state.daily_loss_locked && <span className="rounded-full border px-2 py-0.5 text-[8px] font-bold" style={{ borderColor: "var(--negative)", color: "var(--negative)" }}>DAILY LOSS LOCK</span>}{portfolio.risk_state.stale_open_marks > 0 && <span className="rounded-full border px-2 py-0.5 text-[8px] font-bold" style={{ borderColor: "var(--gold)", color: "var(--gold)" }}>STALE MARK</span>}</div><div className="text-[10px]" style={{ color: "var(--text-mute)" }}>{portfolio.portfolio_id} · {text(portfolio.config.symbol)} · {text(portfolio.config.timeframe_minutes)}m</div></div><div className="text-right text-xs"><div>{money(portfolio.marked_equity)} marked</div><div style={{ color: portfolio.realized_pnl + portfolio.unrealized_pnl_mid >= 0 ? "var(--positive)" : "var(--negative)" }}>{money(portfolio.realized_pnl + portfolio.unrealized_pnl_mid)} total</div></div></div>
