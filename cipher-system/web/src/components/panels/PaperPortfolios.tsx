...
function ScrollTable({ label, className, children }: { label: string; className?: string; children: ReactNode }) {
  return <div className={["overflow-x-auto overflow-y-auto", className].filter(Boolean).join(" ")} role="region" aria-label={label}>{children}</div>;
}

function Stat({ label, value, tone }: { label: string; value: string | number; tone?: string }) {
  return <div className="border px-3 py-2" style={{ borderColor: "var(--line)", background: "var(--panel-2)" }}>
...
export function PaperPortfolios() {
  ...
  if (!data) return <LoadingStatus style={{ color: "var(--text-mute)" }}>Loading shadow portfolios…</LoadingStatus>;
    ...
    <div className="space-y-4" style={{ fontFamily: "var(--font-mono)" }}>
      ...
      <section className="border p-4" style={{ borderColor: executorTone, background: "var(--panel)" }}>
        ...
        <div className="mt-3 grid grid-cols-2 gap-2 md:grid-cols-4 xl:grid-cols-8">
          <Stat label="Marked equity (captured mid)" value={money(data.combined_marked_equity)} />
          <Stat label="Liquidation equity" value={money(data.combined_liquidation_equity)} />
          <Stat label="Today realized (captured fills)" value={money(data.daily_realized_pnl)} tone={data.daily_realized_pnl >= 0 ? "var(--positive)" : "var(--negative)"} />
          <Stat label="Open mid P&L (captured)" value={money(data.combined_unrealized_pnl_mid)} tone={data.combined_unrealized_pnl_mid >= 0 ? "var(--positive)" : "var(--negative)"} />
        ...
        {autopilot?.scheduler.reason && <p className="mt-2 border px-3 py-2 text-[10px]" style={{ borderColor: "var(--gold)", color: "var(--gold)" }}>Blocked: {autopilot.scheduler.reason.replaceAll("_", " ")}</p>}
      ...
      <p className="border px-3 py-2 text-[10px]" style={{ borderColor: "var(--line)", color: "var(--text-mute)" }}>
      ...
      <section className="border p-4" style={{ borderColor: "var(--line)", background: "var(--panel)" }}>
        ...
        <ScrollTable label="Normalized strategy comparison scrollport" className="mt-3"><table aria-label="Normalized strategy comparison" className="w-full text-left text-[10px]"><thead><tr>{["strategy","closed sample","sample status","win rate","average option return","profit factor","rank eligible"].map((h) => <th key={h} className="pr-3 py-1">{h}</th>)}</tr></thead><tbody>{data.normalized_comparison.rows.map((row) => <tr key={row.portfolio_id}><td className="pr-3 py-1 font-semibold">{row.strategy}</td><td className="pr-3 py-1">{row.closed_sample}/{row.minimum_sample}</td><td className="pr-3 py-1" style={{ color: row.sample_status === "USABLE" ? "var(--positive)" : "var(--gold)" }}>{row.sample_status}</td><td className="pr-3 py-1">{percent(row.win_rate)}</td><td className="pr-3 py-1">{percent(row.average_option_return_pct)}</td><td className="pr-3 py-1">{row.profit_factor_on_return_units?.toFixed(2) ?? "—"}</td><td className="pr-3 py-1">{row.rank_eligible ? "yes" : "no"}</td></tr>)}</tbody></table></ScrollTable>
      </section>
      {prospective && <section className="space-y-3 border p-4" style={{ borderColor: "var(--line)", background: "var(--panel)" }}>
        <div>
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 xl:grid-cols-8">
          ...
          <Stat label="Open option mark (captured mid)" value={money(prospective.open_option_mark_pnl)} tone={prospective.open_option_mark_pnl >= 0 ? "var(--positive)" : "var(--negative)"} />
          <Stat label="Spread ceiling" value={`${prospective.option_liquidity_policy.maximum_entry_spread_pct}%`} />
        </div>
        <div className="grid gap-3 lg:grid-cols-2">
          {prospective.programs.map((program) => <div key={program.program_id} className="border p-3" style={{ borderColor: "var(--line)", background: "var(--panel-2)" }}>
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-5"><Stat label="Eligible" value={program.eligible_signals} /><Stat label="Void / audit" value={program.void_signals} tone={program.void_signals ? "var(--negative)" : undefined} /><Stat label="Closed / min" value={`${program.closed_signals}/${program.minimum_sample}`} /><Stat label="Realized option (captured fills)" value={money(program.closed_option_pnl)} tone={program.closed_option_pnl >= 0 ? "var(--positive)" : "var(--negative)"} /><Stat label="Open option mark (captured mid)" value={money(program.open_option_mark_pnl)} tone={program.open_option_mark_pnl >= 0 ? "var(--positive)" : "var(--negative)"} /></div>
        ...
        {prospective.signals.length > 0 && <ScrollTable label="Recent prospective signals scrollport"><table aria-label="Recent prospective signals" className="w-full text-left text-[10px]"><thead><tr>{["signal_bar_at", "ticker", "setup_id", "direction", "status", "outcome", "gross_underlying_return_pct", "option_selection_status"].map((h) => <th key={h} className="pr-3 py-1">{h.replaceAll("_", " ")}</th>)}</tr></thead><tbody>{prospective.signals.map((row, i) => <tr key={text(row.signal_id) + i}>{["signal_bar_at", "ticker", "setup_id", "direction", "status", "outcome"].map((h) => <td key={h} className="pr-3 py-1">{text(row[h])}</td>)}<td className="pr-3 py-1">{percent(row.gross_underlying_return_pct)}</td><td className="pr-3 py-1">{text(row.option_selection_status)}</td></tr>)}</tbody></table></ScrollTable>}
        {prospective.option_legs.length > 0 && <details open><summary className="cursor-pointer text-[10px] font-bold uppercase" style={{ color: "var(--text-mute)" }}>Observed option legs and current marks</summary><ScrollTable label="Prospective option marks scrollport" className="mt-2"><table aria-label="Prospective option marks" className="w-full text-left text-[10px]"><thead><tr>{["contract","status","entry_fill","mark_mid","unrealized_pnl_mid","liquidation_pnl","entry_spread_pct","mark_status"].map((h) => <th key={h} className="pr-3 py-1">{h.replaceAll("_", " ")}</th>)}</tr></thead><tbody>{prospective.option_legs.map((row, i) => <tr key={text(row.leg_id) + i}>{["contract","status","entry_fill","mark_mid","unrealized_pnl_mid","liquidation_pnl","entry_spread_pct","mark_status"].map((h) => <td key={h} className="pr-3 py-1">{text(row[h])}</td>)}</tr>)}</tbody></table></ScrollTable></details>}
        {prospective.observations.length > 0 && <details><summary className="cursor-pointer text-[10px] font-bold uppercase" style={{ color: "var(--text-mute)" }}>Observation coverage and no-signal reasons</summary><ScrollTable label="Prospective observation coverage scrollport" className="mt-2"><table aria-label="Prospective observation coverage" className="w-full text-left text-[10px]"><thead><tr>{["observed_at", "program_id", "ticker", "coverage_status", "decision", "reason", "latest_bar_at"].map((h) => <th key={h} className="pr-3 py-1">{h.replaceAll("_", " ")}</th>)}</tr></thead><tbody>{prospective.observations.map((row, i) => <tr key={text(row.observation_id) + i}>{["observed_at", "program_id", "ticker", "coverage_status", "decision", "reason", "latest_bar_at"].map((h) => <td key={h} className="pr-3 py-1">{text(row[h])}</td>)}</tr>)}</tbody></table></ScrollTable></details>}
      </section>}
      <div className="grid gap-4 xl:grid-cols-2">
        {data.portfolios.map((portfolio) => (
          <details key={portfolio.portfolio_id} className="border p-4" style={{ borderColor: "var(--line)", background: "var(--panel)" }}>
            ...
            <div className="mt-4 space-y-4 border-t pt-3" style={{ borderColor: "var(--line)" }}>
              {portfolio.description && <p className="border px-3 py-2 text-[11px] leading-relaxed" style={{ borderColor: "var(--line)", background: "var(--panel-2)", color: "var(--text)" }}>{portfolio.description}</p>}
              ...
              <div><h3 className="mb-2 text-[10px] font-bold uppercase" style={{ color: "var(--text-mute)" }}>Recent signals and subsequent path</h3>{portfolio.signals.length ? <ScrollTable label={`${portfolio.strategy} recent signals scrollport`}><table aria-label={`${portfolio.strategy} recent signals`} className="w-full text-left text-[10px]"><thead><tr>{["signal_at","setup_id","direction","disposition","skip_reason","outcome","canonical_signal_id","evidence_snapshot_ids","mfe_pct","mae_pct"].map((h) => <th key={h} scope="col" className="pr-3 py-1">{h.replaceAll("_", " ")}</th>)}</tr></thead><tbody>{portfolio.signals.map((row, i) => <tr key={text(row.signal_id) + i}>{["signal_at","setup_id","direction","disposition","skip_reason","outcome","canonical_signal_id","evidence_snapshot_ids"].map((h) => <td key={h} className="pr-3 py-1">{text(row[h])}</td>)}<td className="pr-3 py-1">{percent(row.mfe_pct)}</td><td className="pr-3 py-1">{percent(row.mae_pct)}</td></tr>)}</tbody></table></ScrollTable> : <p className="text-[10px]" style={{ color: "var(--text-mute)" }}>No signals recorded yet.</p>}</div>
              <div><h3 className="mb-2 text-[10px] font-bold uppercase" style={{ color: "var(--text-mute)" }}>Positions / fills / marked P&amp;L</h3>{portfolio.positions.length ? <ScrollTable label={`${portfolio.strategy} positions scrollport`}><table aria-label={`${portfolio.strategy} positions and marked P&L`} className="w-full text-left text-[10px]"><thead><tr>{["status","structure","contract","short_contract","quantity","entry_fill","mark_mid","unrealized_pnl_mid","liquidation_pnl","mark_status","exit_reason","pnl"].map((h) => <th key={h} className="pr-3 py-1">{h.replaceAll("_", " ")}</th>)}</tr></thead><tbody>{portfolio.positions.map((row, i) => <tr key={text(row.position_id) + i}>{["status","structure","contract","short_contract","quantity","entry_fill","mark_mid","unrealized_pnl_mid","liquidation_pnl","mark_status","exit_reason","pnl"].map((h) => <td key={h} className="pr-3 py-1">{text(row[h])}</td>)}</tr>)}</tbody></table></ScrollTable> : <p className="text-[10px]" style={{ color: "var(--text-mute)" }}>No simulated positions yet.</p>}</div>
            </div>
          </details>
        ))}
      </div>
      <div className="border px-3 py-2 text-[10px]" style={{ borderColor: "var(--line)", color: "var(--text-mute)" }}>PAPER ONLY · READ ONLY · EXECUTION CAPABILITY: FALSE · last pass {data.as_of ?? "unavailable"}. Estimated heuristic premiums are not used here. Unknown marks stay labelled, never invented.</div>
    </div>
  );
}
