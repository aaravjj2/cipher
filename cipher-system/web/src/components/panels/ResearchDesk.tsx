...
function CandidateRow({ row, onNavigate }: { row: ResearchCandidate; onNavigate?: (panel: string, ticker?: string) => void }) {
  ...
  return <article className="border" style={{ borderColor: "var(--line)", background: "var(--panel)" }}>
    ...
    {open && <div className="grid gap-4 border-t p-4 lg:grid-cols-[1.2fr_1fr]" style={{ borderColor: "var(--line)" }}>
      <div>
        ...
        <dl className="grid grid-cols-2 gap-x-6 gap-y-2 text-[11px]">
          ...
          <div className="flex justify-between gap-3"><dt>Feed</dt><dd>{feedLabel(row.observed.feed)}</dd></div>
      ...
      <div>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <button type="button" onClick={() => onNavigate?.("Options Terminal", row.ticker)} className="rounded-[4px] border px-2.5 py-1 text-[10px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--gold)]" style={{ borderColor: "var(--line)" }}>Options</button>
          <button type="button" onClick={() => onNavigate?.("Night Vision", row.ticker)} className="rounded-[4px] border px-2.5 py-1 text-[10px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--gold)]" style={{ borderColor: "var(--line)" }}>Chart</button>
          <button type="button" onClick={() => onNavigate?.("Trader Journal", row.ticker)} className="rounded-[4px] border px-2.5 py-1 text-[10px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--gold)]" style={{ borderColor: "var(--line)" }}>Journal</button>
        </div>
      </div>
    </div>}
  </article>;
...
export function ResearchDesk({ onNavigate }: { onNavigate?: (panel: string, ticker?: string) => void }) {
  ...
  if (error) return <div className="border p-4 text-sm" style={{ borderColor: "var(--line)", color: "var(--negative)" }}>{error}</div>;
  ...
  return <div className="flex flex-col gap-4" style={{ fontFamily: "var(--font-mono)" }}>
    <header className="flex flex-wrap items-end justify-between gap-3">
      ...
      <div className="flex border p-1" style={{ borderColor: "var(--line)", background: "var(--panel)" }}>
        {(["intraday", "weekly"] as const).map((value) => <button key={value} type="button" onClick={() => setTab(value)} className="rounded-[4px] px-3 py-1.5 text-[10px] font-bold uppercase focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--gold)]" style={{ background: tab === value ? "var(--nav-active)" : "transparent", color: tab === value ? "var(--text)" : "var(--text-mute)" }}>{value}</button>)}
      </div>
    </header>
    <div className="border px-4 py-3 text-[10px] leading-relaxed" style={{ borderColor: "var(--line)", background: "var(--panel-2)", color: "var(--text-dim)" }}>
    ...
    <div className="grid grid-cols-3 gap-2 sm:max-w-md">
      <div className="border p-3" style={{ borderColor: "var(--line)" }}><span className="block text-[9px] uppercase" style={{ color: "var(--text-mute)" }}>Candidates</span><strong>{candidates.length}</strong></div>
      <div className="border p-3" style={{ borderColor: "var(--line)" }}><span className="block text-[9px] uppercase" style={{ color: "var(--text-mute)" }}>Reviewable</span><strong>{candidates.filter((row) => row.derived.eligible_for_deeper_review).length}</strong></div>
      <div className="border p-3" style={{ borderColor: "var(--line)" }}><span className="block text-[9px] uppercase" style={{ color: "var(--text-mute)" }}>Errors</span><strong>{data.errors?.length ?? 0}</strong></div>
    </div>
    <section className="space-y-2" aria-label={`${tab} research candidates`}>
      ...
      {!candidates.length && <div className="border p-8 text-center text-[11px]" style={{ borderColor: "var(--line)", color: "var(--text-mute)" }}>{data.available ? `No ${tab} candidates passed the current scan.` : "The first scheduled capture has not run yet."}</div>}
    </section>
  </div>;
}
