  ...
  return (
    <div className="flex flex-col gap-4" style={{ fontFamily: "var(--font-mono)" }}>
      ...
      <div className="grid gap-4 xl:grid-cols-4">
        <Card title="4 · Broad market"><div className="space-y-2">{data.market.map((row) => <button key={row.ticker} type="button" onClick={() => onNavigate?.("Ticker Workbench", row.ticker)} className="flex w-full justify-between text-left text-xs"><span>{row.ticker} · {row.price?.toFixed(2) ?? "—"}</span><span style={{ color: (row.day_change_pct ?? 0) >= 0 ? "var(--positive)" : "var(--negative)" }}>{pct(row.day_change_pct)}</span></button>)}</div></Card>
