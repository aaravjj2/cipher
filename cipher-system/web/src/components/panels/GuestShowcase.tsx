...
const DEMOS: Record<string, Demo> = {
  Autopilot: { title: "An AI agent you can audit before you trust", summary: "Premarket research becomes a bounded local-paper decision only after fresh evidence, closed-bar confirmation, contract liquidity, and deterministic risk gates agree. Zero paper trades is a healthy outcome when no setup qualifies.", metrics: [["Phase", "Waiting for confirmation"], ["Authority", "Cipher local paper"], ["Zero-trade", "Healthy"]], rows: ["08:20 ET · Research agents produced thesis and counter-thesis", "09:42 ET · MU confirmation closed above the planned level", "A day with no paper trade can be the correct outcome", "09:43 ET · Modeled fill persisted to Cipher's portfolio only after gates passed", "Exit, reconciliation, and every rejection remain in the audit ledger"], next: "Follow the evidence snapshot into Night Vision, then inspect the local paper trace. This recorded run is illustrative; no brokerage order is possible. Zero paper trades is not a stalled agent." },
  ...
  "Paper Portfolios": { title: "Prospective evidence, never backfilled wins", summary: "Isolated paper ledgers for decisions, rejections, fills, exits, and equity.", metrics: [["Local ledgers", "8"], ["Authority", "Simulation"], ["Backfill", "Never"]], rows: ["Autopilot is one of Cipher's self-managed portfolios", "Every rejection retains its reason", "Option P&L requires an observed or explicitly modeled mark"], next: "Judge strategies from prospective samples and ledger truth." },
...
function LiveAgentRecord() {
  ...
  useEffect(() => {
    let cancelled = false;
    fetch(hostedApiUrl("/api/agent-showcase"), {
      credentials: "include",
