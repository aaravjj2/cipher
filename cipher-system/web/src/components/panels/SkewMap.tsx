  ...
  return <section className="space-y-3 font-mono" data-testid="skew-map" data-guest-panel="Skew Map" data-guest-source="live">
    ...
    <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_300px]">
      ...
      <aside className="border border-[var(--line)] bg-[var(--panel)] p-4">
        {point ? <><div className="flex items-start justify-between gap-2"><div><h2 className="text-lg font-semibold">{point.ticker}</h2><p className="text-[10px] text-[var(--text-mute)]">{point.sector} · {point.front_expiry ?? "expiry unavailable"} · {point.sessions}/20 sessions</p></div><span className="border border-[var(--line)] px-2 py-1 text-[9px] uppercase text-[var(--gold)]">{point.sessions < 20 ? "PROVISIONAL" : point.quality}</span></div>
