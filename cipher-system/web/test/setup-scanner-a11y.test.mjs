...
test("Flash capture requires source freshness for Live and displays the full date", () => {
  assert.match(scanner, /agentic\?\.freshness\?\.state === "current"/);
  assert.match(scanner, /Stale capture/);
  assert.match(scanner, /historical snapshot, not a live signal/);
  assert.match(scanner, /year: "numeric", month: "short", day: "numeric"/);
});
...
test("Setup Scanner CSV and history controls have accessible names", () => {
  ...
  assert.match(scanner, /role="group"/);
  assert.doesNotMatch(scanner, /role="option"/);
