...
def build_scope(
    ...
    *,
    ...
    """Apply the original price-only scope logic to an explicit partition set.

    Accepting explicit paths lets canonical dataset verification use the exact
    same implementation that produced the protected scope artifact. Path text
    is preserved rather than resolved so baseline partition identities remain
    byte-for-byte comparable through the compatibility symlink.
    """

    selected_paths = tuple(sorted(Path(path) for path in paths))
    if not selected_paths:
        raise ValueError("no normalized Alpaca recovery partitions")
    ...
    for path in selected_paths:
        ...
        for ticker, group in frame.groupby("ticker"):
            rows.append(
                {
                    "ticker": ticker,
                    "date": group["timestamp"].iloc[0].date().isoformat(),
                    "bars": len(group),
                    "close": float(group.sort_values("timestamp").iloc[-1]["close"]),
                    "path": str(path),
                }
            )
    ...
    for ticker, group in daily.groupby("ticker", sort=True):
        ...
        for row in group.itertuples(index=False):
            ...
            decision = require_price_only_market_day(
                observed_bars=int(row.bars),
            ...
            result = {
                ...
                "reason": (
                    "first_or_calendar_restart"
                    if eligible and ratio is None
                    else decision["price_continuity"]["reason"]
                ),
            }
            ...
            if eligible:
                ...
                if len(stretch) >= MIN_STRETCH:
                    stretches.append(
                        {
                            "ticker": ticker,
                            "start": stretch[0]["date"],
                            "end": stretch[-1]["date"],
                            "sessions": len(stretch),
                        }
                    )
                stretch = []
    ...
    common_eligible = [
        {"date": day, "tickers": sorted(tickers), "count": len(tickers)}
        for day, tickers in sorted(common.items())
    ]
    ...
    return {
        ...
        "feed": "sip",
        "panel": list(PANEL),
        "period": f"{start or 'all'}..{end or 'all'}",
        "normalized_partitions": len(selected_paths),
        "normalized_partition_sha256": {
            str(path): hashlib.sha256(path.read_bytes()).hexdigest() for path in selected_paths
        },
        "gate": {
        ...
        "full_gate_module_sha256": hashlib.sha256(code.read_bytes()).hexdigest(),
        "daily_results": results,
        "stretches_at_least_52_sessions": stretches,
        "common_eligible_by_day": common_eligible,
...
def main() -> None:
    ...
    payload = build_scope(paths, start=args.start, end=args.end)
    ...
    print(
        json.dumps(
            {
                "path": str(output),
                "partitions": payload["normalized_partitions"],
                "stretches": len(payload["stretches_at_least_52_sessions"]),
                "max_common": max(
                    (item["count"] for item in payload["common_eligible_by_day"]),
                    default=0,
                ),
            },
            indent=2,
