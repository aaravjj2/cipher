...
# The dedicated workspace owns its imports. Do not fall back to the parent
# repository, which could silently mix shared code into this experiment.
for path in (str(CIPHER_ROOT), str(CIPHER_ROOT / "core")):
...
def _candidate_indices(
    ...
    *,
    ...
    for i, state in enumerate(states):
        # `signal_gate`, not `in_window`: the former is mode-aware and admits every RTH bar
        # under "Full Session", while the latter is only the end-of-day arming window. Using
        # `in_window` here meant Full Session mode still discarded every signal outside the
        # final `arm_minutes`, so the mode existed in the detector and did nothing in a run.
        if i >= len(bars) or not state.signal_gate:
            continue
...
def _trade_for_signal(
    ...
    *,
    ...
    for index in range(entry_index, len(bars)):
        ...
        if evaluation_end is not None and local.date() > evaluation_end:
            break
    ...
    if direction == "LONG":
        ...
        entry = raw_entry - tick_size
        exit = raw_exit + tick_size
        # A short's return is P&L divided by capital committed at entry, not by
        # the later exit price. ``entry / exit - 1`` overstates profitable shorts
        # and understates losing shorts because it changes the denominator with
        # the outcome. Use the entry mark consistently for both pre-cost and net.
        gross = ((raw_entry - raw_exit) / raw_entry) * 100.0
        net = ((entry - exit) / entry) * 100.0
    return PineTrade(
        symbol=symbol,
        direction=direction,
...
def _summary(trades: list[PineTrade], bars: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        # Renamed from `gross_sum_return_pct`, which summed `net_return_pct` and so reported a
        # post-slippage number under a name meaning pre-cost everywhere in trading. It was also
        # the only "gross" figure emitted, so the cost drag appeared nowhere in the output.
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--json-out", default=str(CIPHER_ROOT / "results" / "obsidian_pine_ytd_2026.json"))
    parser.add_argument("--csv-out", default=str(CIPHER_ROOT / "results" / "obsidian_pine_ytd_2026.csv"))
