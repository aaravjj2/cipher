...
class BarState:
    ...
    coiling: bool
    in_window: bool
    # Whether signals are permitted on this bar at all. `in_window` is specifically the
    # end-of-day arming window; this is the mode-aware gate, true for every RTH bar under
    # "Full Session". They were conflated: the detector applied `gate` to the CLPS flags but
    # only published `in_window`, so a consumer filtering on `in_window` silently kept the
    # last-30-minutes restriction even in Full Session mode — which made that mode inert.
    signal_gate: bool
    hot: bool
